import * as React from "react";
// Minimal local implementation of `cva` and `VariantProps` to avoid requiring the external module.
// This provides a simple runtime function compatible with the basic usage in this file.
type VariantConfig = Record<string, any>;
function cva(base?: string, _config?: { variants?: any; defaultVariants?: any }) {
  return (options?: Record<string, any>) => {
    const classes: string[] = [];
    if (base) classes.push(base);
    if (options) {
      // if variant provided, append as a class name (this mirrors basic usage in this file)
      const v = options.variant;
      if (v) classes.push(String(v));
    }
    return classes.join(" ");
  };
}
type VariantProps<T> = Record<string, any>;

import { cn } from "./utils";

const alertVariants = cva(
  "relative w-full rounded-lg border px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
        default: "bg-card text-card-foreground",
        destructive: "text-destructive bg-card [&>svg]:text-current *:data-[slot=alert-description]:text-destructive/90",
      },
    },
    defaultVariants: {
      variant: "default",
    },
  }
);

function Alert({ className, variant, ...props }: React.ComponentProps<"div"> & VariantProps<typeof alertVariants>) {
  return <div data-slot="alert" role="alert" className={cn(alertVariants({ variant }), className)} {...props} />;
}

function AlertTitle({ className, ...props }: React.ComponentProps<"div">) {
  return <div data-slot="alert-title" className={cn("col-start-2 line-clamp-1 min-h-4 font-medium tracking-tight", className)} {...props} />;
}

function AlertDescription({ className, ...props }: React.ComponentProps<"div">) {
  return <div data-slot="alert-description" className={cn("text-muted-foreground col-start-2 grid justify-items-start gap-1 text-sm [&_p]:leading-relaxed", className)} {...props} />;
}

export { Alert, AlertTitle, AlertDescription };
